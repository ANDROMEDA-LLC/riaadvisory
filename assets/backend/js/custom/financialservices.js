$(document).ready(function () {
    ...
    $(document).on('click', 'button#sil', function (e) {
        ...
        alertify.confirm("Are you sure you want to delete it ?", function (e) {
            if (e) {
                $.ajax({
                    type: "Post",
                    url: SITE_URL + "/admin/financialservices/financialservicesSil/" + ID,
                    cache: false,
                    dataType: "json",
                    data: {},
    ...
    $('#form').submit(function (e) {
        ...
        if (duzenleme == 0) {//ekleme
            $.ajax({
                type: "POST",
                url: SITE_URL + "admin/financialservices/financialservicesEkle",
                cache: false,
                dataType: "json",
            ...
            $.ajax({
                type: "POST",
                url: SITE_URL + "admin/financialservices/financialservicesDuzenle",
                cache: false,
                dataType: "json",
                data: new FormData(this),
                async: false,
    ...
    $(document).on('click', 'button#duzenle', function (e) {
        ...
        $.ajax({
            type: "post",
            url: SITE_URL + "admin/financialservices/financialservicesBul/" + ID,
            dataType: "json",
            cache: false,
            data: {},
            success: function (cevap) {
                if (cevap.hata) {
                    ...
                    if (cevap.result) {
                        ...
                        $("#image-holder").empty();
                        $("#image-holder").prepend('<img id="theImg" src="' + SITE_URL + 'uploads/financialservices/' + cevap.result.resim_yol + '" class="thumb-image img-responsive"/>');
